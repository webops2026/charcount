...
export function ManuscriptEditor({ text }: ManuscriptEditorProps) {
  ...
  const drawManuscript = useCallback((pageIndex: number) => {
    ...
    const hookWidth = cellSize * 0.5;
    ctx.beginPath();
    ctx.moveTo(centerX - hookWidth, upperY);
    ctx.lineTo(centerX + hookWidth, upperY);
    ctx.stroke();
    // 縦線（上から魚尾まで）
    ctx.beginPath();
    ...
    ctx.beginPath();
    ctx.moveTo(centerX - hookWidth, lowerY);
    ctx.lineTo(centerX + hookWidth, lowerY);
    ctx.stroke();
    // 縦線（魚尾から下まで）
    ctx.beginPath();
